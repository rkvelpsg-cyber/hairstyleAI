    ...
    async function tick() {
      ...
      try {
        ...
        if (faceLm?.length) {
          ...
          if (yaw < -0.12) raw = "left";
          else if (yaw > 0.12) raw = "right";
          else raw = "front";

          confidence = Math.min(0.98, 0.65 + Math.abs(yaw) * 0.7);
        } else if (poseLm?.length) {
          const nose = poseLm[0];
          const leftShoulder = poseLm[11];
          const rightShoulder = poseLm[12];
          const shouldersVisible =
            (leftShoulder?.visibility ?? 0) > 0.35 &&
            (rightShoulder?.visibility ?? 0) > 0.35;
          const noseVisible = (nose?.visibility ?? 0) > 0.25;
          ...
          setState((s) => ({
            ...
            status:
              error instanceof Error
                ? error.message
                : "Orientation tracker unavailable",
          }));
        }
      }
      timer = setTimeout(tick, 140);
